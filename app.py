"""
Stock Buy/Sell Ladder — multi-broker CSV import + strategy recommendations.
...
except ModuleNotFoundError:  # pragma: no cover - cloud misconfig fallback
    px = None
...
# Streamlit Cloud may run app.py from repo root OR from stock_buy_sell/.
def _resolve_root() -> Path:
    ...
    # Package layout: .../stock_buy_sell/app.py → project root is parent
    if (here / "engine.py").is_file() and (here.parent / "stock_buy_sell").is_dir():
        return here.parent
    # Flat repo layout: app.py + engine.py at same level (common on Cloud)
    if (here / "engine.py").is_file():
        return here
    return here.parent
...
for _p in (_HERE, _ROOT, _USER_ROOT):
    s = str(_p)
    if s not in sys.path:
        sys.path.insert(0, s)

# Prefer flat imports when engine.py sits next to app.py (your GitHub / Streamlit Cloud layout).
# Fall back to package imports for local monorepo: stock_buy_sell/app.py
if (_HERE / "engine.py").is_file():
    from engine import (  # type: ignore  # noqa: E402
        action_summary,
        build_recommendations,
        load_trades_csv,
    )
    from positions import (  # type: ignore  # noqa: E402
        default_sample_path,
        get_last_import_source,
        positions_to_dataframe,
    )
    from strategy import ladder_reference_table  # type: ignore  # noqa: E402
    from brokers import supported_brokers_table  # type: ignore  # noqa: E402
else:
    ...
    from stock_buy_sell.positions import (  # noqa: E402
        default_sample_path,
        get_last_import_source,
        positions_to_dataframe,
    )
    from stock_buy_sell.strategy import ladder_reference_table  # noqa: E402
    from stock_buy_sell.brokers import supported_brokers_table  # noqa: E402
...
def _load_trades_from_upload(uploaded) -> pd.DataFrame:
    ...
    return load_trades_csv(io.BytesIO(raw))


@st.cache_data(show_spinner="Fetching live prices…", ttl=120)
def _cached_build(trades_csv_bytes: bytes, mtime_key: str):
    trades = load_trades_csv(io.BytesIO(trades_csv_bytes))
    source = get_last_import_source()
    positions, recs, display = build_recommendations(trades, fetch_prices=True)
    return trades, positions, recs, display, source


def main() -> None:
    st.title("📉 Stock Buy/Sell Ladder")
    st.caption(
        "Import broker transaction / orders CSVs · evaluate each ticker against the "
    ...
    with st.sidebar:
        ...
        uploaded = st.file_uploader(
            ...
            type=["csv"],
            help=(
                "Exports from Fidelity, Schwab, IBKR, Robinhood, E*TRADE, Webull, Moomoo, "
                "TD/ToS, Vanguard, or any CSV with Symbol/Date/Qty/Price (+ optional Side). "
                "Format and title rows are auto-detected. Equity trades only — options skipped."
            ),
        )
        ...
        with st.expander("Supported brokers", expanded=False):
            st.dataframe(
                pd.DataFrame(supported_brokers_table()),
                hide_index=True,
    ...
    if uploaded is not None:
        ...
        st.info("Upload a broker transaction or orders CSV in the sidebar to get started.")
        st.stop()

    mtime_key = source_label
    try:
        trades, positions, recs, display, import_source = _cached_build(
            trades_bytes, mtime_key
        )
    ...
    st.success(
        f"Loaded **{len(trades)}** stock trades "
        f"(detected: **{import_source or 'Unknown'}**) from `{source_label}` "
    ...
    with tab_recs:
        if display.empty:
            ...
            if not chart_df.empty:
                ...
                if px is None:
                    st.warning(
                        "Charts need **plotly**. Add `plotly>=5.18` to your repo’s "
                        "`requirements.txt` (repo root), commit, then **Reboot** the app."
                    )
                    st.bar_chart(chart_df.set_index("Symbol")["Change %"])
                else:
                    fig = px.bar(
                        chart_df,
                        x="Symbol",
    ...
    with tab_holds:
        ...
        if pos_df.empty:
            ...
            if not display.empty and display["Market Value"].notna().any():
                ...
                if not pie.empty:
                    if px is None:
                        st.bar_chart(pie.set_index("Symbol")["Market Value"])
                    else:
                        fig = px.pie(
                            pie,
                            names="Symbol",
...
1. Import broker fills (buys positive, sells negative after normalization).
