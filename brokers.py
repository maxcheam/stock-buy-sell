...
def _parse_etrade_orders(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not all([sym_c, fill_c, desc_c, time_c]):
        raise ValueError(
            f"E*TRADE Orders: missing columns. Found: {list(df.columns)}"
        )
    ...
    out = _finalize(rows, "E*TRADE Orders")
    if out.empty and (skipped_options or skipped_unfilled):
        raise ValueError(
            "E*TRADE Orders: no filled **stock** trades found. "
...
def supported_brokers_table() -> list[dict[str, str]]:
    return [
        ...
        {
            "Broker": "E*TRADE Orders",
            "Notes": "Orders CSV (live or paper): Symbol/Status/Fill/Description; filled stocks only",
        },
