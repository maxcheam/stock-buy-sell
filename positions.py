"""Import multi-broker transaction CSVs and derive open positions."""
...
try:
    from .brokers import load_broker_csv, supported_brokers_table  # noqa: F401
except ImportError:  # flat deploy (Streamlit Cloud)
    from brokers import load_broker_csv, supported_brokers_table  # type: ignore  # noqa: F401
...
# Preferred sample files under input/ (first match wins)
_SAMPLE_CANDIDATES = (
    "Orders_eTrade.csv",
...
# Last detected source label (for UI); set by load_trades_csv
_LAST_SOURCE: str = ""
...
def load_trades_csv(path_or_buffer, *, broker: str | None = None) -> pd.DataFrame:
    """Load a broker (or generic) transaction/orders CSV.
...
def default_sample_path(root: Path | None = None) -> Path:
    """Locate a bundled sample CSV (dev tree, next to .exe, or PyInstaller extract)."""
    import os
    import sys
    ...
    if user_dir:
        search_roots.append(Path(user_dir))
    ...
    # Fallback path (may not exist — UI disables sample checkbox)
    return search_roots[0] / "input" / _SAMPLE_CANDIDATES[0]
