"""Unit tests for ladder strategy + StockEvents position math (no network)."""
...
from stock_buy_sell.stockevents import derive_positions, load_stockevents_csv
...
"""
    trades = load_stockevents_csv(io.StringIO(csv))
...
def test_sample_csv_loads_if_present():
    sample = Path(__file__).resolve().parent.parent / "input" / "stock_events_transactions_2026-08-02.csv"
    if not sample.exists():
        pytest.skip("sample CSV not present")
    trades = load_stockevents_csv(sample)
    assert not trades.empty
    assert set(["Symbol", "Date", "Quantity", "Price"]).issubset(trades.columns)
    positions = derive_positions(trades)
    assert len(positions) >= 1
    # Fully closed symbols should not appear
    for p in positions:
        assert p.shares > 0
