...
## Input CSV (StockEvents + major brokers)

Upload a **transaction history CSV**. Format is **auto-detected**.

| Broker | Typical export |
|--------|----------------|
| StockEvents | `Symbol, Date, Quantity (+/−), Price` |
...
| Generic | Symbol + Date + Qty + Price [+ Side] |

- Equity **buys/sells only** (options, dividends, transfers are skipped when detected)
- Positive qty = buy, negative = sell (after normalization)
